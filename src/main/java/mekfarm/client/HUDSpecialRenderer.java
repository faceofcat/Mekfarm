...
import mekfarm.machines.BaseElectricEntity;
import mekfarm.machines.BaseOrientedBlock;
...
/**
 * Created by CF on 2016-11-25.
 */
public class HUDSpecialRenderer<T extends BaseElectricEntity> extends TileEntitySpecialRenderer<T> {
    @Override
    public void renderTileEntityAt(T te, double x, double y, double z, float partialTicks, int destroyStage) {
        if (te.hasCapability(MekfarmCapabilities.CAPABILITY_MACHINE_INFO, null)) {
            IMachineInfo info = (IMachineInfo) te.getCapability(MekfarmCapabilities.CAPABILITY_MACHINE_INFO, null);
            List<ColoredTextLine> lines = info.getHUDLines();
            ...
            if ((lines != null) && (lines.size() > 0)
                ...
                if ((side == EnumFacing.DOWN) || (side == EnumFacing.UP)) {
                    side = BaseOrientedBlock.getFacingFromEntity(te.getPos(), this.rendererDispatcher.entityX, this.rendererDispatcher.entityZ);
                }
    ...
    private void renderText(List<ColoredTextLine> messages, float scale)
    {
        ...
        for (ColoredTextLine ctl : messages)
        {
            ...
            if (ctl.alignment == ColoredTextLine.TextAlignment.LEFT) {
                ...
                if (ctl.alignment == ColoredTextLine.TextAlignment.RIGHT) {
                    font.drawString(line,
                            padding + 1 + blockSize - 2 - textWidth,
                            y + 2, (ctl.color == null) ? 16777215 : ctl.color.getRGB());
                } else if (ctl.alignment == ColoredTextLine.TextAlignment.CENTER) {
                    font.drawString(line,
